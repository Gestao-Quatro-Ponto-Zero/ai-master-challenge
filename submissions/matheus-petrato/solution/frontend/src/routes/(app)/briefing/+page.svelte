<script lang="ts">
	...
	let kpis = $state<any[]>([]);
	let hotDeals = $state<any[]>([]);
	let riskDeals = $state<any[]>([]);
	let insights = $state<string[]>([]);
	let teamSnapshot = $state<any[]>([]);
	...
	const normalizeDeals = (items: any[] = []) =>
		items.map((deal) => ({
	...
	onMount(async () => {
		try {
			const data = await api.briefing.get();
			kpis = (data.kpis || []).map((kpi: any) => ({
				...
				icon:
					(kpi.label ?? '').toLowerCase().includes('risco')
						...
						? Target
						: (kpi.label ?? '').toLowerCase().includes('ciclo')
						? Calendar
						: TrendingUp
			}));
			hotDeals = normalizeDeals(data.hot_deals || data.hotDeals || []);
			riskDeals = normalizeDeals(data.risk_deals || data.riskDeals || data.critical_deals || []);
			insights = data.insights || [];

			const snapshot = data.team_snapshot || data.teamSnapshot;
			if (snapshot) {
				teamSnapshot = [
					{ label: 'Vendedores ativos', value: formatValue(snapshot.active_agents ?? snapshot.activeAgents ?? '-'), icon: Users },
					{ label: 'Deals criticos', value: formatValue(snapshot.critical_deals ?? snapshot.criticalDeals ?? '-'), icon: AlertTriangle },
					{
						label: 'Pipeline por regiao',
						value: snapshot.region_focus ?? snapshot.regionFocus ?? '—',
						icon: TrendingUp
					}
				];
			}
		} catch (err) {
			console.warn('Falha ao carregar briefing:', err);
		}
	});
</script>
