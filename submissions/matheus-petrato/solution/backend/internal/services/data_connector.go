...
// FetchDealsResult holds paginated deals and total count
type FetchDealsResult struct {
	Items []DealDTO `json:"items"`
	Total int       `json:"total"`
}

func (s *DataConnector) FetchDeals(ctx context.Context, filter DealFilter) (*FetchDealsResult, error) {
	query := `
		SELECT 
			d.id, COALESCE(acc.name, 'Unknown'), ds.score, d.stage, 
			EXTRACT(DAY FROM (NOW() - d.engage_date)) as days,
			COALESCE(d.close_value, p.sales_price, 0), ds.label
		FROM deals d
	...
	// Map frontend status to backend labels: stalled = warm + cold (deals without score also count as stalled)
	if filter.Status != "" {
		if filter.Status == "stalled" {
			query += " AND (ds.label IN ('warm', 'cold') OR ds.label IS NULL)"
		} else {
	...
	if filter.Region != "" {
		query += fmt.Sprintf(" AND sa.regional_office_id = (SELECT id FROM regional_offices WHERE name = $%d)", argID)
	...
	query += fmt.Sprintf(" ORDER BY COALESCE(ds.score, 0) DESC NULLS LAST LIMIT %d OFFSET %d", limit, offset)
	...
	for rows.Next() {
		...
		} else if d.Status == "zombie" || d.Status == "risk" {
			d.Status = "risk"
	...
	return &FetchDealsResult{Items: deals, Total: total}, nil
}
...
func (s *DataConnector) FetchDealDetail(ctx context.Context, dealID uuid.UUID) (map[string]any, error) {
	...
	err := s.pool.QueryRow(ctx, `
		SELECT 
			...
			COALESCE(d.close_value, p.sales_price, 0),
			EXTRACT(DAY FROM (NOW() - d.engage_date)),
		...
		WHERE d.id = $1`, dealID).Scan(
			&oppID, &acc, &product, &stage, &score, &label, &value, &daysOpt, &factorsRaw, &owner,
		)
	...
	nextActions := []string{"Agendar call de decisão com o sponsor"}
	if status == "risk" {
		nextActions = []string{"Reengajar sponsor", "Agendar call de alinhamento"}
	} else if status == "hot" {
		nextActions = []string{"Agendar call final", "Enviar proposta"}
	}
