...
func (h *ChatHandler) runAgent(ctx context.Context, userID uuid.UUID, req ChatRequest) (uuid.UUID, string, error) {
	...
	// Fetch User Context for Tools
	var role string
	var salesAgentID, managerID *uuid.UUID
	err := h.ConvService.GetPool().QueryRow(ctx, 
		"SELECT role, sales_agent_id, manager_id FROM users WHERE id = $1", 
		userID).Scan(&role, &salesAgentID, &managerID)
	
	if err != nil {
		return convID, "", fmt.Errorf("user context not found")
	}
	...
	// Set dynamic IDs for tools
	sID := ""
	mID := ""
	if role == "manager" && managerID != nil {
		mID = managerID.String()
	} else if role == "seller" && salesAgentID != nil {
		sID = salesAgentID.String()
	}

	al.Tools.Register(&services.GetMyDealsTool{BaseAgentTool: services.BaseAgentTool{Connector: h.DataConnector, SellerID: sID, ManagerID: mID}})
	al.Tools.Register(&services.GetDealDetailTool{BaseAgentTool: services.BaseAgentTool{Connector: h.DataConnector, SellerID: sID, ManagerID: mID}})
	al.Tools.Register(&services.SearchDealsTool{BaseAgentTool: services.BaseAgentTool{Connector: h.DataConnector, SellerID: sID, ManagerID: mID}})
