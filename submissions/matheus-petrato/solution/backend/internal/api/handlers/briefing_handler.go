...
func (h *BriefingHandler) getManagerBriefing(c *fiber.Ctx, userID uuid.UUID) error {
	...
	var snapshot struct {
		...
		CriticalDeals  int     `json:"critical_deals"`
		RegionFocus    string  `json:"region_focus"`
	}
	...
	// Count critical (zombie) deals for the team
	h.pool.QueryRow(ctx, `
		SELECT COUNT(*) FROM deals d
		JOIN sales_agents sa ON d.sales_agent_id = sa.id
		JOIN deal_scores ds ON d.id = ds.deal_id
		WHERE sa.manager_id = $1 AND ds.label = 'zombie'
	`, managerID).Scan(&snapshot.CriticalDeals)

	// Top region by pipeline value (may have no rows if no deals)
	snapshot.RegionFocus = "N/A"
	_ = h.pool.QueryRow(ctx, `
		SELECT COALESCE(ro.name, 'N/A') FROM (
			SELECT sa.regional_office_id, SUM(COALESCE(d.close_value, p.sales_price, 0)) as total
			FROM deals d
			JOIN sales_agents sa ON d.sales_agent_id = sa.id
			JOIN products p ON d.product_id = p.id
			WHERE sa.manager_id = $1 AND d.stage IN ('Prospecting', 'Engaging')
			GROUP BY sa.regional_office_id
			ORDER BY total DESC LIMIT 1
		) t
		LEFT JOIN regional_offices ro ON ro.id = t.regional_office_id
	`, managerID).Scan(&snapshot.RegionFocus)
	...
	rows, _ := h.pool.Query(ctx, `
		SELECT d.id, COALESCE(acc.name, 'Unknown'), ds.score, sa.name, d.stage,
	...
	for rows.Next() {
		...
		var days float64
		var value float64
		rows.Scan(&id, &acc, &score, &seller, &stage, &days, &value)
