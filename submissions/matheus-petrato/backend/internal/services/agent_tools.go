...
func (t *GetMyDealsTool) Execute(ctx context.Context, args map[string]any) (*agent.ToolResult, error) {
	...
	// Map 'risk' back to 'zombie' for internal query if needed, 
	// but DataConnector's FetchDeals should handle the mapping if we aligned it.
	deals, err := t.Connector.FetchDeals(ctx, DealFilter{
		Status:   status,
		SellerID: t.AgentID.String(),
	})
	if err != nil {
		return nil, err
	}

	content, _ := json.Marshal(deals)
