...
import type { TokenUsageByModel } from '../../services/tokenUsageTrackerService';
...
interface Props {
  rows: TokenUsageByModel[];
  loading: boolean;
}

export default function TokenUsageByModelTable({ rows, loading }: Props) {
  ...
  return (
    <div className="bg-slate-900 border border-white/5 rounded-xl overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-4 border-b border-white/5">
        ...
        <h3 className="text-white font-semibold text-sm">Token Usage by Model</h3>
      </div>
      ...
      {loading ? (
        ...
        <div className="py-12 text-center text-slate-600 text-sm">No data for selected period</div>
      ) : (
