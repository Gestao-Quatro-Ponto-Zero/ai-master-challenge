...
import { formatIntegerPtBr, formatStructuredLabel } from "@/lib/format";
...
export function MetricCard({ metric }: { metric: Metric }) {
  return <article className="panel p-5"><p className="data-label">{metric.label}</p><p className="mt-3 font-mono text-3xl font-semibold tracking-tight">{formatIntegerPtBr(metric.value)}</p><p className="mt-2 text-sm text-muted">{metric.context}</p></article>;
}

const styles: Record<string, string> = {
  READY_FOR_REVIEW: "border-blue/30 bg-blue/10 text-blue", PILOT_ONLY: "border-gold/40 bg-gold/10 text-amber-800", UNDERPOWERED: "border-orange/40 bg-orange/10 text-orange-800", NOT_FEASIBLE: "border-slate-300 bg-slate-100 text-slate-700", UNTESTED: "border-slate-300 bg-white text-slate-700", P1: "border-orange/40 bg-orange/10 text-orange-800", P2: "border-gold/40 bg-gold/10 text-amber-800", P3: "border-blue/30 bg-blue/10 text-blue", P4: "border-slate-300 bg-slate-100 text-slate-700", HIGH: "border-pink/30 bg-pink/10 text-pink", MEDIUM: "border-gold/30 bg-gold/10 text-amber-800", LOW: "border-slate-300 bg-slate-100 text-slate-700", ROBUST: "border-olive/40 bg-olive/10 text-olive", SENSITIVE: "border-gold/40 bg-gold/10 text-amber-800"
};

export function StatusBadge({ value }: { value: string }) {
  return <span className={`inline-flex rounded-full border px-2.5 py-1 text-[0.68rem] font-bold uppercase tracking-wide ${styles[value] ?? "border-slate-300 bg-slate-50 text-slate-700"}`}>{formatStructuredLabel(value)}</span>;
}
...
export function LimitationCallout({ children, title = "Limite de interpretação" }: { children: React.ReactNode; title?: string }) {
...
function printable(value: JsonValue | undefined): string {
  if (value === undefined || value === null) return "Não disponível";
  if (typeof value === "object") return JSON.stringify(value, null, 2);
  return String(value);
}

const explainFields: Array<[keyof ExplainData, string]> = [
  ["what_was_observed", "O que foi observado"], ["why_it_appears_here", "Por que aparece aqui"], ["evidence", "Evidência"], ["population", "População"], ["denominator", "Denominador"], ["quality", "Qualidade"], ["stability", "Estabilidade"], ["limitations", "Limitações"], ["authorized_next_step", "Próximo passo autorizado"], ["prohibited_interpretation", "Interpretação proibida"]
];

export function ExplainThis({ data, label = "Explicar esta evidência" }: { data: ExplainData; label?: string }) {
...
export function ProvenancePanel({ value }: { value: JsonValue }) {
  return <details className="text-sm"><summary className="cursor-pointer font-semibold text-blue">Ver proveniência técnica</summary><pre className="mt-2 max-h-72 overflow-auto rounded-xl bg-ink p-4 text-xs text-slate-100">{printable(value)}</pre></details>;
}
...
export function ErrorState({ title, message, action }: { title: string; message: string; action?: () => void }) {
  return <div className="panel flex min-h-64 flex-col items-center justify-center border-orange/30 p-8 text-center" role="alert"><DatabaseZap className="text-orange" size={36} aria-hidden /><h2 className="mt-4 text-xl font-semibold">{title}</h2><p className="mt-2 max-w-md text-sm text-muted">{message}</p>{action && <button className="button-primary mt-5" onClick={action}>Tentar novamente</button>}</div>;
}

export function DataFreshness({ cutoff }: { cutoff: string }) {
  return <div className="inline-flex items-center gap-2 text-xs text-muted"><Info size={14} aria-hidden /><span>Dados históricos até 31 de dez. de 2024 · snapshot local fixo</span><span className="sr-only">Data-limite técnica {cutoff}</span></div>;
}
