...
Os dados não sustentam o diagnóstico operacional descrito literalmente no brief. O Dataset 1 contém **{d1['rows']:,} tickets**, não aproximadamente 30 mil, e não possui o horário de abertura do ticket. `First Response Time` e `Time to Resolution` são timestamps, não durações. Entre {d1['paired_timestamp_rows']:,} tickets com ambos os campos, **{pct(d1['negative_response_to_resolution_rate'])}** têm resolução anterior à primeira resposta. Portanto, FRT, TTR, desperdício de horas e ROI observado não podem ser calculados de forma válida a partir desse arquivo.
...
3. Usar o Dataset 1 apenas para volumetria, cobertura, padrões textuais, status e associações descritivas claramente limitadas.
4. Tratar qualquer ROI como cenário parametrizado, nunca como economia observada.
...
4. Apresentar o classificador como prova técnica de triagem, não como modelo validado para a G4.
