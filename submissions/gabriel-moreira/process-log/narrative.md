...
**5. Badges de contagem por estado inconsistentes.** A contagem de oportunidades por ESTADO nas abas só aparecia quando > 0 (um estado com zero oportunidades simplesmente não mostrava número, em vez de mostrar "(0)"). Corrigido para inicializar os cinco estados em zero antes de contar — achado durante a revisão com a skill `/vercel-react-best-practices`.

**Verificação de ponta a ponta:** rodei a aplicação real (API + Vite dev server) no navegador como Manager, Supervisor e Sales Agent — confirmando visualmente isolamento de escopo, ocultação da aba Gestão, download do CSV restrito a Manager, e os números batendo com os documentados em `docs/architecture.md` (ex.: distribuição de CONFIANÇA A/B/C/D bateu exatamente com as porcentagens documentadas: 89/2089≈4,3% A, 372/2089≈17,8% B, 337/2089≈16,1% C, 1291/2089≈61,8% D).

**Limitação de ambiente encontrada — `docker compose up --build` não pôde ser verificado de ponta a ponta nesta sessão.** O ambiente sandbox onde o Claude Code rodou bloqueia pulls de imagem do Docker Hub (`docker pull python:3.12-slim`, `node:20-slim` e até `alpine:latest` ficaram pendurados sem progredir, mesmo depois de o Docker Desktop subir corretamente). Fiz o que dava para verificar sem a rede: `docker compose config` confirma que o YAML é válido e que os `context`/`dockerfile` de cada serviço resolvem para os caminhos certos; revisei os três Dockerfiles à mão linha a linha (inclusive corrigindo `npm install`→`npm ci` para usar o lockfile, e adicionando `.dockerignore` em `submissions/gabriel-moreira/` e em `solution/` depois de perceber que `COPY solution/scoring /scoring` copiaria os `.venv/` locais para dentro da imagem sem isso); e testei de ponta a ponta o caminho equivalente sem Docker (`make dev-api` + `make dev-web`, os mesmos processos que os Dockerfiles empacotam), incluindo navegação real no browser como Manager/Supervisor/Sales Agent. **Fica pendente para quem revisar:** rodar `docker compose up --build` num ambiente com acesso normal à internet antes de considerar a entrega finalizada — a lógica está correta até onde dá para verificar sem a rede, mas a partida via Docker em si não foi confirmada.

**Pendente ao fim desta sessão:** `git add -f` (task 7.6, staging — não commit, que fica para confirmação explícita), e a verificação de `docker compose up --build` mencionada acima.
