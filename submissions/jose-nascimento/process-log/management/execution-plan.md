...
- **Status:** `CONCLUDED` (2026-08-28) — decisões D1–D9 com regras PRÉ-especificadas antes do backtest (`process-log/decisions/iteration-04-watchlist-decisions.md`); implementação validada pelo executor: exit 0; **29 PASS / 0 WARN / 0 FAIL**; idempotência byte-a-byte (3 execuções + CWD diferente); FAIL estrutural sem traceback; 3 verificações manuais independentes (MV1a/b reativação com/sem próximo evento; MV2 rank shifts current vs lifecycle; MV3 onboarding/watchlist); recálculo independente do backtest 3/3 cutoffs e de recorrência/reativação; no-leakage audit coluna a coluna (checks G6b); commit `feat: prioritize accounts by lifecycle and validated risk signals`. Review gate 3x da It04 `PENDING` (ledger separado, item B3 do checklist).
