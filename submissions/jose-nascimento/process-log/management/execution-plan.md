...
- **Última atualização:** 2026-08-28 (fim da Iteração 06 — pipeline em 1 comando: `run.sh` + `Makefile` + `requirements.txt` minimizado (pandas==3.0.5, matplotlib==3.11.1) + verificador `solution/src/06_verify_pipeline.py` (67 PASS / 0 FAIL) + `solution/README.md`; validações: 2× `./run.sh` e `make all` byte-idênticos (46/46 outputs), CWD diferente, 3 cenários de FAIL (dado/schema/deps) sem traceback, `make clean-derived` seguro com regeneração byte-idêntica, bash -n, compile/imports, tempo ~64–66s e pico ~176MB medidos (ru_maxrss, aproximação declarada); commit `build: one-command reproducible pipeline`; **It06 `CONCLUDED`** — ver `process-log/reports/iteration-06-reproducibility-report.md`; **review gate 3x da It06 `PENDING`**; It07–It10 `PENDING`)
...
- **Status:** `CONCLUDED` (2026-08-28) — pipeline em 1 comando implementado e validado: `run.sh` (executável, `set -euo pipefail`, path próprio, preflight Python/deps/data, estágios 01→05 com propagação de exit code, verificador ao final, tempo/memória medidos) + `Makefile` (`all` = `./run.sh`; `verify`; `stage-01..05`; `clean-derived` com lista explícita) + `requirements.txt` minimizado (pandas==3.0.5, matplotlib==3.11.1 — pins exatos; duckdb/seaborn/jupyterlab/numpy explícito removidos por inspeção de imports) + verificador `solution/src/06_verify_pipeline.py` (67 PASS / 0 FAIL; manifesto/parse/contratos commitados/higiene/sanidade; exit 1 com diagnóstico estruturado, sem traceback) + `solution/README.md` (setup, outputs, estrutura, tempo/memória, troubleshooting, lentes, licença). Validações: `./run.sh` 2× e `make all` byte-idênticos (46/46 outputs MD5 iguais ao commitado); CWD diferente idêntico; 3 cenários de FAIL (dado ausente / schema quebrado / python-deps ausentes) com exit nonzero, mensagem útil e 0 tracebacks; `make clean-derived` apaga só a lista explícita (raw/process-log intactos) e regenera byte-idêntico; `make verify` OK; bash -n; compile/imports; tempo ~64–66s e pico ~176 MB (ru_maxrss, aproximação declarada); `git diff --check` limpo. Commit: `build: one-command reproducible pipeline` (mensagem exigida no prompt; a prevista no rascunho do plano, "feat: one-command reproducible pipeline with validation", foi substituída — esta linha reflete o commit real). **Review gate 3x da It06 `PENDING`** (dispara em seguida). Ver `process-log/reports/iteration-06-reproducibility-report.md` e `process-log/decisions/iteration-06-reproducibility-decisions.md`.
