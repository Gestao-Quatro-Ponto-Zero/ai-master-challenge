...
- **Git history:** 26 commits do candidato no HEAD `9e60315` (commit do process log); **31 no fechamento da It08** (fixer do gate em 5 commits, incl. a consolidação do snapshot) + 5 commits de base do repo oficial; todos os hashes citados abaixo resolvem (`git rev-parse`).
...
26 commits do candidato no HEAD do process log (`9e60315`); **31 no fechamento da It08** (fixer do gate em 5 commits, incl. a consolidação do snapshot); + 5 de base do repo oficial. Hashes resolvem via `git rev-parse` (verificador G9). Mapeamento por iteração (commit de etapa → commit de correção do gate):
...
| It08 | `docs: consolidate AI process log and evidence` (`9e60315`) → fixer do gate em 5 commits (`78933e6` `docs: reconcile process log review evidence` · `a93178b` · `4c48713`/`0cb8375` · consolidação do snapshot) |
