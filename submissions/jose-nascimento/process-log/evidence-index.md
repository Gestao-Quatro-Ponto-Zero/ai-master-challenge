...
- **Git history:** 26 commits do candidato no HEAD `9e60315` (commit do process log); **29 no fechamento da It08** (fixer do gate em 3 commits) + 5 commits de base do repo oficial; todos os hashes citados abaixo resolvem (`git rev-parse`).
...
26 commits do candidato no HEAD do process log (`9e60315`); **29 no fechamento da It08** (fixer do gate em 3 commits); + 5 de base do repo oficial. Hashes resolvem via `git rev-parse` (verificador G9). Mapeamento por iteração (commit de etapa → commit de correção do gate):
...
| It08 | `docs: consolidate AI process log and evidence` (`9e60315`) → fix do gate `docs: reconcile process log review evidence` (`78933e6`) + ajustes do fixer (`a93178b` · `4c48713`) |
