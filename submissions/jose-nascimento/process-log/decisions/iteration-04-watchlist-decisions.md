...
**Nota de transparência (registro honesto, pós-gate It04):** este arquivo foi commitado no MESMO commit do código e dos outputs (`adbbad7`) — diferentemente da It03 (hipóteses `8cb93c3` commitadas antes do código `9e02e18`), a **cronologia git não prova** que as decisões precederam a primeira execução. A pré-especificação é atestada pelo conteúdo interno (números de exploração que divergem dos finais — ex.: KM 90d 0,72/180d 0,64/mediana não alcançada vs finais 0,653/0,476/187d — e P75 9.751, Spearman 0,575) e por auto-relato do executor. A prática It03 (commitar decisões antes do código) é retomada nas It05+.
...
**Problema:** 37 de 61 episódios de reativação não têm evento posterior observado; concluir "reativar funciona" seria erro de censura (muitas reativações são recentes: 26 de 61 flags = 42,6% em out-dez/2024 — parcela substancial, não maioria; entre as 37 censuradas, 20 = 54,1% são recentes, o que concentra a censura nos episódios novos).

**Decisão (D7):** por episódio de reativação (61): gap desde o evento anterior (mediana 45d, n=35 — 26 flags são o 1º evento da conta, sem gap), tempo até o próximo evento com **Kaplan-Meier com censura no corte** e taxas com denominador explícito de follow-up (10/35 episódios com ≥90d de follow-up têm próximo evento ≤90d = 28,6%). **Valores finais (do script validado; derivados em runtime no relatório, com gates de âncora G13): sobrevivência em 90d = 0,653; em 180d = 0,476; mediana = 187 dias (alcançada).** A exploração inicial (censura KM incompleta — erro documentado no report da iteração §4.1) estimava 0,72/0,64/mediana não alcançada; esses valores estão **superados** e não são usados em nenhuma conclusão. Nenhuma taxa é chamada de "sucesso de reativação"; nenhum valor em US$ é atribuído a reativação (sem ligação demonstrável com receita — contrato §5).
...
**Decisão (D9):** tabelas `t11_account_lifecycle.csv` (500 contas), `t12_reactivation_recurrence.csv` (distribuições de eventos/reativações/episódios), `t13_state_cycles.csv` (ciclos reais vs lentes), `t14_backtest_temporal.csv` (regras×cutoffs), `t14b_backtest_detail.csv` (flags por conta×cutoff para auditoria), `t15_priority_segments.csv` (segmentos N/$), `t16_watchlist_top20.csv` (watchlist), `t17_rank_comparison.csv` (current vs lifecycle). Gráficos (prefixo `It04_`): (a) recorrência×reativação; (b) lentes de ciclo (evento vs assinatura vs estado); (c) current MRR × lifecycle proxy (top-20 destacados); (d) lift do backtest por cutoff com intervalos. It03 não possui nenhum desses. **Pós-gate It04 (pruning):** apenas os gráficos essenciais são commitados — `It04_c_lifecycle_vs_current_mrr.png` e `It04_d_backtest_lift.png`; `It04_a` (recorrência×reativação) e `It04_b` (lentes de ciclo) foram substituídos pelas tabelas `t12` e `t13` (que contêm exatamente os mesmos números), conforme decisão do gate 3x — ver `process-log/reviews/iteration-04-review-summary.md`.