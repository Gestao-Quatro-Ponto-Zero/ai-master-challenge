...
|---|---|---|---|
| MV1 | D09 — uso antes do início da assinatura (19.142; 76,6%) | `python3` heredoc: merge `feature_usage` × `subscriptions.start_date`; contagem de `usage_date < start_date`; conferência do exemplo `S-0fcf7d` (usage 2023-02-22 vs start 2024-11-23) | PASS — 19.142 de 25.000 (76,6%); 4.783 assinaturas distintas afetadas |
| MV2 | C01/C02 — divergência de fontes de churn (35/277/125) | `python3` heredoc: conjuntos `account_id` de `accounts.churn_flag`, `churn_events` e `subscriptions.churn_flag`; exemplo concreto `A-00bed1` (1 evento, 0 assinaturas churn_flag, accounts.churn_flag=False) | PASS — 110 flag / 352 contas com evento / 600 eventos; 35 flag-sem-evento; 277 evento-sem-flag; 125 evento-sem-assinatura-churn |
| MV3 | S03 + D07 — 21 `usage_id` duplicados; 1.077 tickets antes do signup | `python3` heredoc: `duplicated(usage_id)`; confirmação de que os 21 IDs apontam para assinaturas distintas (reuso); merge tickets × accounts e contagem de `submitted_at < signup_date`; exemplo `T-0024de` (submitted 2023-07-27 vs signup 2023-12-16) | PASS — 21 IDs duplicados (21/21 com >1 assinatura); 1.077 tickets antes do signup |
