...
2. **Regras obrigatórias do contrato:** lente de eventos para diagnóstico (primeiro evento por conta, censura no corte 2024-12); lente de assinaturas para MRR; winner para MRR de conta; features de risco apenas com dados ≤ data índice (alvo vs feature no mesmo mês proibido — contrato §8); uso/tickets com variante bruta vs alinhada declarada (contrato §9); CSAT/reason/feedback apenas sugestivos (contrato §10); nenhuma comparação entre lentes (contrato §4).
