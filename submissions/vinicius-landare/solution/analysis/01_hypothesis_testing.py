...
def df_to_json(dataframe, filename):
    """Salva DataFrame como JSON, resetando index se necessário."""
    path = os.path.join(OUTPUT_DIR, filename)
    dataframe.reset_index().to_json(path, orient="records", indent=2, force_ascii=False)
    print(f"  -> {filename}")
...
df_to_json(h1_disclosure, "h1_disclosure_type.json")
...
with open(os.path.join(OUTPUT_DIR, "general_stats.json"), "w") as f:
