...
import { Inbox, CheckCircle, Lightbulb, LogOut, Loader2 } from "lucide-react";
import Image from "next/image";

const NAV = [
  { href: "/agente", label: "Fila de Tickets", icon: Inbox },
  { href: "/agente/resolvidos", label: "Resolvidos", icon: CheckCircle },
  { href: "/agente/sugestor", label: "Base RAG — Soluções", icon: Lightbulb },
];
...
export default function AgenteLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!loading && user && user.perfil !== "agente") router.replace("/gestor");
  }, [user, loading, router]);
    ...
    <div className="flex h-screen overflow-hidden">
      <aside className="w-64 bg-sidebar flex flex-col shrink-0">
        ...
        <nav className="flex-1 px-3 space-y-1 mt-4">
          {NAV.map((item) => {
            const active = pathname === item.href || (item.href !== "/agente" && pathname.startsWith(item.href));
            return (
